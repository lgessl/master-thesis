\documentclass[10.5pt, a4paper, openright, twoside]{report}
\usepackage{layout}
\usepackage[margin=102pt
%, textwidth=390pt
]{geometry}
...
\begin{document}
    ...
    \begin{abstract}
        ...
        risk-assessment tool for DLBCL and a simple clinical test, fails to identify 
        a high-risk DLBCL subpopulation that is 
        large enough to trigger research and to enable clinical trials for new treatments 
        ...
        progression-free survival (PFS) below two years with a prevalence of at least \num{10}\% 
        and a significantly higher precision than the IPI and to show this on independent data. It 
        also deals with the question under which circumstances we can do so reliably.
        ...
        data sets and 
        shows that we can indeed deliver a model that has the desired properties and is transferable 
        between measurement protocols. Analysis after 
        freezing all trained models and unlocking the test data for all of them suggests that, for 
        a reliable 
        internal validation and high test performance, the following factors are crucial:
        ...
        we can identify a sufficiently large DLBCL subpopulation with poor survival that
        holds significantly more high-risk patients than the high-risk group identified by the 
        IPI. With more data and new and 
        more accurately measured features in the future, we expect to be able to further improve the 
        performance of models like ours.
    \end{abstract}
