...
$y_i$ is the $i$-th entry of the response vector $y$. MMML-Predict is all about getting 
...
In this subsection, we work with binary response $y \in \RR^n$. Both ordinary linar models and 
logistic regression models are generalized linear models (GLMs). In 
...
and hence $\sum{j \in J} \beta_j = 0$. In this situation, the linear predictor 
\begin{align}
...
hazard function than the population with another value of $X$ if $X$ predictive power with respect 
to $T$. Formally, however, it is hard to define a conditional hazard function: $X$ may very well 
