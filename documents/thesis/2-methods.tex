...
\begin{itemize}
    \item fit the model to the train cohort subject to $h$ and provide validated predictions for 
        ...
        subject to $h$ to data that does not include this sample. 
        ...
        $\hat{y}_\text{train} = \text{cv}(h)$ of the same shape as $y_\text{train}$. 
    \item Afterwards, we use the cross-validated predictions to calculate the validation error 
...
$\text{cv}$ is not a deterministic mapping. The procedure to calculate validated predictions often 
involves randomness, e.g. how we split the training samples into folds in a cross validation, and 
many fitting algorithms like that of random forests resort to randomness. Therefore, we 
treat $\text{cv}(h)$ as a random variable. It is a well known property of independent,
...
Usually, not all $\text{err}(y_\text{train}, \text{cv}(h)), h \in H$, are i.i.d. because some models
models specified by $H$ are better suited for the problem than others.
Still, subsets of $H$ can easlily deliver models with validation errors close to being i.i.d. 
We often try out a bunch of hyperparameter tuples across which the fit models 
do not essentially differ and hence their validated predictions and errors are essentially the 
same, in other words: identically distributed.
All that makes the validated errors differ is independent randomness. This particularly holds true 
for the subset of $H$ that specifies unsuited, merely guessing models. In both cases, with growing 
size of such a subset of $H$, the minimal validated error among the models in this subset will get 
lower and lower.
...
Similarly, given a cohort, we threshold the IPI in such a way that it maximizes the precision under 
...
If we 
measure the same $p$ features of the $n$ patients in $\mathbf{x} \in \RR^{n \times p}$ again with 
...
We are not done, yet, with the loss function: the coordinate descent minimizes 
Eq.\ \eqref{eq:loss-glm-no-lasso} with a regularization term add. Before we discuss this, we want 
...
linear predictor with an increasing function. Given a cohort of samples, we are stil most 
...
\begin{split}
    ...
    &= c \gamma_1 + (f \circ (f_1, \ldots, f_m))(x) = c \gamma_1 + g(x_i)
\end{split}
\end{align}
...
perspective, where output ordering matters most, we can say: the better the approximation, the more 
