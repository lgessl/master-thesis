...
performance of the chosen model convincingly. Section \ref{sec:train-val-test} will lay out the 
...
The design of MMML-Predict with its training and test cohort, where the developers of the predictor 
...
As we increase $t_i$ and thereby decrease the prevalence, the obtained 
precision values belonging to two adjacent output thresholds $t_i$, $t_j$ differ more and more; 
differences of up to \num{0.5} are possible. But requiring a prevalence of at least 17\% caps the 
differences of precision values dating from two adjacent $t_i$, $t_j$ at
\begin{align}
...
output. To obtain a binary classification for data measured under another 
protocol, we need to re-calibrate the output threshold of the linear model. However, this 
...
\paragraph{Features in multiple formats}
...
\begin{itemize}
    ...
    \item As a special case of this, for already existing models, we can either include their continuous 
        ...
        cell-of-origin signatures, whose output is continuous to begin with, are thresholded into 
...
\subsubsection{Tuning the definition of ``high risk''}
...
We want to show a use case of the zero-sum constraint where only part of the features suffer from 
...
measures the time \textit{at} which the event of interest occurs (``event'') or the time 
...
Formally, however, it is hard to define a conditional hazard function: we often have $P(X = x) = 0$, 
e.g., if $X$ follows a density-defined distribution, hence we condition on an event of probability 
$0$ when we write 
$P(t \leq T < t + \Delta t \cond T \geq t, X = x)$ -- it is hard to define this in a 
straightforward way. In practice, the measurements in $X$ often fluctuate around their true 
...
There are more analogies to GLMs. In Eq.\ \eqref{eq:cox-proportional-factor}, we also compose a 
...
We want to use our samples $(\mathbf{x}_i, y_i)$, $i = 1, \ldots, n$, to learn a tree $T$ that does 
...
\end{align}
With respect to the impurity measure, there are only $n(R) - 1$ split points per feature of 
interest. Therefore, when splitting $R$, we need to consider no more than $p \cdot n(R)$ split 
points.
...
Thus, in addition to bagging, random forests strive to reduce the correlation between the trees 
...
of 
Alg.\ \ref{alg:nested-pcv}, i.e., we repeatedly call \textsc{NestedPseudoValidation} as we vary $h \in H_0$
...
set $H_0 = H_{0, 1} \times H_{0, 2}$. Compared to non-nested models, we try out a considerably 
...
forests, which, at the core, predict by thresholding single features, are highly sensitive to shifts in 
