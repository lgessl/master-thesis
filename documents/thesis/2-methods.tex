...
\paragraph{Early models trained on the same data set}
...
We wanted to design the software used to generate the results of this thesis in such a way that it 
...
together with a \href{https://lgessl.github.io/patroklos/}{website}.
\begin{description}
    \item[Training] \texttt{patroklos} allows the user to quickly specify model architectures and 
        attach hyperparamters tuples, including model-agnostic hyperparameters, tuples. It provides 
        decorators to transform functions that fit merely one model for a single hyperparameter 
        tuple into functions that fit and validate models for a variety of hyperparameter tuples. 
        It supports fitting the models with the support of the R packages \texttt{zeroSum} 
        ...
        endowed with elastic-net regularization and -- unlike \texttt{glmnet} -- the zero-sum 
        constraint. The time-critical work happens in a fast, well-structured C++ backbone. 
        \texttt{ranger} fits random forest fast, also thanks to a performant C++ backbone. 
    \item[Validation and testing] If fitting functions from existing R packages calculate a 
        validated error of 
        their fit models at all, they do it in a plethora of ways, usually using some standard 
        error measure for the model class they are concerned with derived from the loss function 
        they minimize; e.g., \texttt{zeroSum} uses the binomial deviance for logistic models and 
        \texttt{ranger} uses the classicatication error. \texttt{patroklos} unifies validation 
        by calculating the same error measure across all models and more calculates more statistics
        ...
        thresholding those models with continuous output.
    \item[Modular, extendible design] \texttt{patroklos} defines function interfaces so the user 
        can easily wrap fitting functions implemented in other R packages or by the user to make 
        them \texttt{patroklos}-compliant. With two R6 classes, \texttt{Model} and \texttt{Data}, we 
        strive to abstract models from the data in such a way that the user can easily $H$ to new 
        data.
    \item[Meta analysis] \texttt{patroklos} provides tools to analyze how validated and test error
        depend for model architectures depend and allows to highlight supgroups of model 
        architectures to disclose fundamentally flawed validated errors.
\end{description}
