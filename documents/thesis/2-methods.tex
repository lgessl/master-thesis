...
\end{align}
Multiplied with the regularization strength $\lambda \geq 0$, we can add it to the loss function
$L$ from Eq.\ \ref{eq:loss-glm-no-lasso} and Eq.\ \ref{eq:cox-loss} to obtain the regularized 
loss function
\begin{align}
...
predictor matrix. We can use the weights to standardize $X$ by setting $v_j$ to an estimate of the 
standard deviation of feature $j$. Why 
...
Eq.\ \ref{eq:enet-weights} becomes smaller and smaller. For $\lambda$ large enough, there exists 
a unique, trivial solution with $\hbeta = 0$. For the above-mentioned coordinate descent, given 
...
We minimize Eq.\ \eqref{eq:enet-weights} for $\lambda$ from this sequence in decreasing order. This gives rise 
...
How can we use our samples $(\mathbf{x}_i, y_i)$, $i = 1, \ldots, n$, to learn a tree $T$? While we can 
...
\begin{align}
    R^{(1)}_{j, s} = \{x \in R: \mathbf{x}_j \leq s\} \text{ and } R^{(2)}_{j, s} = \{x \in R: \mathbf{x}_j > s\}.
\end{align}
...
data $(\mathbf{x}_i, y_i)$, $i = 1, \ldots, n$. Instead providing every 
